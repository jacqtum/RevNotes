...
CREATE OR REPLACE PROCEDURE FINDUSER(FNAME IN VARCHAR2, LNAME IN VARCHAR2)
AS
BEGIN
    SELECT U.USER_ID, U.USER_FNAME, U.USER_LNAME, BA.BANK_ACCOUNT_ID, BA.BANK_ACCOUNT_BALANCE
    FROM USERS U
    JOIN BANKACCOUNTS BA ON U.USER_ID = BA.USER_ID
    WHERE U.USER_FNAME = FNAME AND U.USER_LNAME = LNAME;
END;
